...
contract EntityContract is Ownable {
    ...
    function getActiveLicenseeSLs() external returns (address[] memory) {
    ...
    function getActiveLicensorSLs() external returns (address[] memory) {
        ManagerContract manager;
    ...
    function getTerminatedLicenseeSLs() external returns (address[] memory) {
        ManagerContract manager;
        address[] memory result = new address[](licenseeSLs.length);
        uint256 k=0;
        for (uint256 i = 0; i < licenseeSLs.length; i++) {
    ...
    function getTerminatedLicensorSLs() external returns (address[] memory) {
        ManagerContract manager;
        address[] memory result = new address[](licensorSLs.length);
        uint256 k=0;
        for (uint256 i = 0; i < licensorSLs.length; i++) {
